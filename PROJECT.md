...
| `SettingsModal.jsx` | 分页（通用/模型密钥/**工具**/更新/系统）；工具页三卡片（git/pnpm/node 来源/版本/路径 + 缺失时单装/重建 shim，进度日志+强停，**点击即清空各自日志**）；更新页双管线 + 进度日志 + **按管线强制终止**；harness 源码仓库**只读展示**（自定义需手改 settings.json）；系统页配置文件双路径（客户端 settings.json + harness 多模态目录 ~/.dsh/settings.yaml，各带打开按钮）；harness 无自动更新 |
...
| 位置 | 内容 |
|---|---|
| `%APPDATA%\dsh-client\settings.json` | config：workspace / harnessDir / harnessRepo（设置页只读）/ clientUpdateRepo / clientAutoUpdate / provider / model / activeCustom / reasoningEffort / maxTokens / customModels[] / dsApiKey / dsBaseUrl / theme（harnessAutoUpdate 已废弃，更新仅手动）。**原子写**；`.bak` 上一版轮换 / `.bad` 坏文件隔离；支持手改（尾逗号/BOM 宽松恢复） |
