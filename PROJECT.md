...
| 文件 | 职责 |
|---|---|
| `main.mjs` | 窗口创建（亮色初始 titleBarOverlay + 主题联动 `dsh:setNativeTheme`）；全部 `ipcMain.handle`；`killOrphanRuntimes`（启动清残留防写句柄占用）；`scheduleReconnect`（崩溃指数退避重连×5）；**`handleRuntimeDeath`**（runtime 死亡分诊：ERR_MODULE_NOT_FOUND → 一次会话内自动清构建戳+重装重建 harness，进度推入 harnessProgress 通道；其余走 scheduleReconnect；死亡头部写入 crash.log）；`runHarnessPipeline`（更新流水线入口，ensureTools 自动装便携工具链；手动"重装并重建"清构建戳强制完整重建）；`runClientUpdate` / `silentSelfUpdate`（客户端自更新：空闲守卫 → 先客户端后 harness；界面改动 reload / 主进程改动 relaunch）；右键菜单注册（HKCU 注册表 `Directory\shell\DSHClient`，命令带 `--workspace "%V"`）；渲染错误写 `userData/crash.log`；窗口 ready-to-show 写 `client-boot-ok`（loader 回滚判据）；`appRoot` 解析（dev=项目目录，打包=exe 目录，harness/workspace 默认相对它） |
...
| 文件 | 职责 |
|---|---|
| `App.jsx` | 状态根：snapshot/runtime 事件订阅；5s 状态对账（防 HMR 后徽章卡死）；主题（localStorage + 原生按钮联动，**默认 light**）；**UI 缩放**（zoom state + applyZoom 设置 CSS `--font-scale`，Ctrl 快捷键/滚轮，纯文字缩放；**勿改回 webFrame.setZoomFactor**——全局缩放会破坏布局结构，用户明确要求仅文本）；**双更新管线的进度行缓冲**（harnessLines/clientLines 各 60 行，SettingsModal 从 props 读——弹窗关闭重开不丢日志）；runtime 死亡错误带 stderrHead（模块名在错误头）；switchModel（非 DeepSeek 官方模型自动关思考并记忆原值）；计划闭环回调 |
| `ChatPanel.jsx` | 最大组件：消息流渲染（Markdown-lite/工具卡片/图片 Lightbox+保存/复制按钮）；**Lightbox 支持保存图片**（`dsh:saveImage` IPC 接收 base64 或附件名，主进程 dialog.showSaveDialog → 下载目录默认）；**输入框图片预览悬停保存按钮**（与删除按钮并列）；LiveThinking（思考折叠卡自动沉底）+ live-output（输出实时渲染在外部）；**构建/计划模式**（按钮+Tab 切换，input-card 色条 class）；composer（发送=打断中则先 interrupt；防连击 800ms）；work-dots（12 点扫描工作指示）；jump-bottom（滚动>240px 出现，autoScroll 距底 40px 内才跟随）；计划提示条（plan-ready → applyPlan 组合 EXECUTE PLAN 消息）。**状态区**：标题下 meta-line（小圆点+就绪/工作中/启动中/未连接 · 消息数 · 当前模型名）；欢迎页为 WhaleMark 大图标 |
...
| `SettingsModal.jsx` | **左侧分页设置**（通用/模型密钥/更新/系统 四 tab）：通用=工作区/harness路径/**界面缩放**（70%~150%，**纯文字缩放**——CSS `--font-scale` 驱动 html font-size，87 处 font-size 已全部 rem 化，布局/图标/间距不变；Ctrl+滚轮/= /− /0 快捷键，localStorage 持久，main.jsx 挂载前应用无闪烁）；密钥=DeepSeek 官方 key/BaseURL（激活自定义模型时显示提示但仍可预配置）；更新=客户端/harness 双管线（**进度日志由 App 级 buffers 传入——关闭弹窗再开不丢日志**；busy 状态来自 status IPC，渲染层重载后也能恢复"执行中"显示；「重装并重建」清构建戳强制完整重装）；系统=右键菜单注册+配置文件路径。**默认主题 = light**（localStorage 'dsh-theme' 覆盖；窗口初始色也是亮色，避免新用户暗色闪烁） |
