...
| `dsh-runtime.mjs` | **核心类 DshRuntime**：spawn runtime（优先构建产物，回退 tsx；系统缺 node 用 `ELECTRON_RUN_AS_NODE`）；JSON-RPC 收发；**会话聚合**（echo 去重保留图片预览、usage 累计、tool 配对、**userInterrupted 抑制打断 abort 红条**）；`applyStreamChunk` 实时预览；审批队列；图片落盘 + `dshimg://` 协议；`sessions.json` 持久化（stripBlocks 瘦身：文本 50000/工具结果 5000/参数 8000 字符；**逐消息 usage 持久化**——数据层已留，明细 UI 暂缓）；resume 失败回退新 ID；`updateConfig` 时为空 workspace 的旧会话回填烙印；`focusWorkspaceSession`（切换工作区后聚焦/新建该工作区会话，复用未使用的空会话）；**子代理系统**：`subagent.started/finished` 给子会话打标（subagent/parentId/subagentStatus/subagentIndex/viewed 全持久化，标号粘性防重拉顶号，遗留数据按时间回填），`orderedSessions` 过滤隐藏不出侧栏，snapshot 附 `subagents[]` 列表，`deleteSubagent`（防误删：运行中被删的子代理后续事件以隐藏身份+原标号重建，finished 的删除即生效），`markSubagentViewed` 未读标记 |
